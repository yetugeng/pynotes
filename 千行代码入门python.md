...
if isinstance(L, list): print("L is list")  #推荐
```
...
## 数字常量
```python
1234, -1234, 0, 999999999              # 整数 int
0o177, 0x9ff, 0X9FF, 0b101010          # 八进制、十六进制、二进制数字 (不同进制的也是整数)
1.23, 1., 3.14e-10, 4E210, 4.0e+210    # 浮点数 float
3+4j, 3.0+4.0j, 3J                     # 复数常量，也可以用complex(real, image)来创建

# 这两个可以互换数据类型 str <--> int
hex(I), oct(I), bin(I)             `    # 将十进制数转化为十六进制、八进制、二进制表示的“字符串”
int(str, base)                         # 将字符串转化为整数，base为进制数。进制数要和字符串对应。
...
x[i], x[i:j:k], x(……)                        # 索引(越界报错)、分片(越界不报错)、调用
int(3.14),  float(3)                         # 强制类型转换
```
问题:x(...)啥意思？x[i:j:k]这个呢？
...
Decimal("0.01") + Decimal("0.02")        # 返回 Decimal("0.03")
...
s.add('x')         t.remove('H')                  # 增加/删除一个item
s.update([10,37,42])                              # 利用[......]更新s集合，效果类似add
...
# 效果同下面表达式
t = set("Hello")                                  # 创建一个唯一字符的集合，返回{'o', 'e', 'H', 'l'}
```
问题：t.update([10,37,42])  怎么更新的？
## 集合frozenset，不可变对象
```python
"""
...
s1+s2, s1*3, s[i], s[i:j], len(s)       # 字符串操作：拼接，重复，索引，切片，取长度
...
str.ljust(width,'0')# 获取固定长度，右对齐，左边不足用0补齐

str.index('t')				# 从左起，找到第一个字符的索引
str.find('t',start,end)		# 查找字符串，可以指定起始及结束位置搜索，也是只显示第一个匹配的的索引
...
str.lstrip()	# 移除首部空格
str.rstrip()	# 移除尾部空格
str.strip('d')	# 移除首尾的字符d
str.lstrip('d')	# 移除首部的字符d
str.rstrip('d')	# 移除尾部的字符d
...
```
## and或or总是返回对象(左边的对象或右边的对象) 且具有短路求值的特性
```python
1 or 2 or 3                            # 返回 1
1 and 2 and 3                          # 返回 3
```
...
```
## 可以使用**name**进行模块的单元测试:当模块为顶层执行文件时值为'**main**' 当模块被导入时为模块名
```python
